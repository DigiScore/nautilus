...
class TrainModel:
    ...
    def train(self):
        for epoch in tqdm(range(self.epochs), desc='epochs'):
            ...
            for i in tqdm(range(0, self.total_songs, self.batch_song), desc='MUSIC'):
                ...
                for nnet_steps in tqdm(range(0, len(index_shuffled), self.batch_nnet_size)):
                    steps_nnet += 1
...
def piano_roll_to_pretty_midi(piano_roll, fs=100, program=0):
    ...
    pm.adjust_times([0, length], [0, length*8]) # This will effectively slow it down to 800% of its original length
